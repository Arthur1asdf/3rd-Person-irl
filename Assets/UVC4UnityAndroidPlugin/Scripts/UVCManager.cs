...
namespace Serenegiant.UVC
{
    ...
    public class UVCManager : MonoBehaviour
    {
        ...
        internal bool _pendingStart = false;
        internal int _pendingStartDeviceId;
        internal IntPtr _pendingStartPtr;
        internal int _pendingStartWidth;
        internal int _pendingStartHeight;
		...
		*/
        ...
        public class CameraInfo
        {
            ...
            public UVCVideoSize FindNearest(bool preferH264, UInt32 width, UInt32 height)
            {
                ...
                foreach (var frameType in frameTypes)
                {
                    ...
                    if (found.IsValid)
                    {
                        ...
                        Debug.Log($"Found format: {found.Width}x{found.Height}, FrameType: {found.FrameType}");
                        break;
                    }
                }

                // debug section start
                Debug.Log($"=== Supported Formats for Camera ({SupportedSize.Length} formats) ===");
                foreach (var size in SupportedSize)
                {
                    string formatName = "Unknown";
                    int frameTypeInt = (int)size.FrameType;

                    // Check for known UVC4Android types
                    if (size.FrameType == 0x000000) formatName = "YUYV/YUY2 (Uncompressed)";
                    else if (size.FrameType == 0x000007) formatName = "MJPEG";
                    else if (size.FrameType == 0x000014) formatName = "H264";
                    else if (size.FrameType == 0x030011) formatName = "H264_FRAME";
                    else formatName = $"CUSTOM (0x{size.FrameType:X6})";

                    Debug.Log($"  {size.Width}x{size.Height} @ {formatName} [FrameType: {size.FrameType}]");
                }
                // debug section end
            ...
            private IEnumerator OnRenderBeforeSceneRendering()
            {
                ...
                for (; activeId != 0;)
                {
                    ...
                    if (_manager._pendingStart)
                    {
                        _manager._pendingStart = false;
                        Start(_manager._pendingStartDeviceId,
                              _manager._pendingStartPtr,
                              _manager._pendingStartWidth,
                              _manager._pendingStartHeight);
                        Debug.Log("UVC: Called native Start() from render thread");
                    }
            ...
            private IEnumerator OnRender()
            {
                ...
                for (; activeId != 0;)
                {
                    ...
                    // If Start() hasn't been called from render thread yet, do it now
                    if (_manager._pendingStart)
                    {
                        _manager._pendingStart = false;
                        Start(_manager._pendingStartDeviceId,
        ...
        private void StartPreview(UVCDevice device, UVCVideoSize size)
        {
            ...
            if ((info != null) && !info.IsPreviewing)
            {
                ...
                Debug.Log($"Attempting to start preview with format: {size.FrameType} at {size.Width}x{size.Height}");

                // Try the suggested format first
                int resizeResult = Resize(device.id, size.FrameType, size.Width, size.Height);

                // If that fails with the weird 65541 format, try forcing MJPEG
                if (resizeResult != 0 || size.FrameType == 65541)
                {
                    Debug.LogWarning($"Format {size.FrameType} failed or is unusual. Trying MJPEG instead...");

                    // Try MJPEG at same resolution
                    resizeResult = Resize(device.id, 0x000007, size.Width, size.Height);
                    if (resizeResult == 0)
                    {
                        size.FrameType = 0x000007; // Update to MJPEG
                        Debug.Log("Successfully switched to MJPEG format");
                    }
                    else
                    {
                        // Try MJPEG at 640x480
                        resizeResult = Resize(device.id, 0x000007, 640, 480);
                        if (resizeResult == 0)
                        {
                            size.Width = 640;
                            size.Height = 480;
                            size.FrameType = 0x000007;
                            Debug.Log("Successfully switched to MJPEG at 640x480");
                        }
                    }
                }
                ...
                if (resizeResult == 0)
                {
                    ...
                    mainContext.Post(__ =>
                    {
                        // Determine texture dimensions based on actual format
                        int textureWidth, textureHeight;

                        if (size.FrameType == 0x000000) // YUY2
                        {
                            textureWidth = Convert.ToInt32(size.Width / 2);
                            textureHeight = Convert.ToInt32(size.Height);
                            Debug.Log($"Creating YUY2 texture: {textureWidth}x{textureHeight}");
                        }
                        else // MJPEG or other - full width
                        {
                            textureWidth = Convert.ToInt32(size.Width);
                            textureHeight = Convert.ToInt32(size.Height);
                            Debug.Log($"Creating MJPEG/RGB texture: {textureWidth}x{textureHeight}");
                        }

                        var tex = new RenderTexture(1280, 720, 0, RenderTextureFormat.ARGB32);
                        tex.useMipMap = false;
                        tex.autoGenerateMips = false;
                        tex.filterMode = FilterMode.Bilinear;
                        tex.wrapMode = TextureWrapMode.Clamp;
                        // Critical: don't let Unity apply gamma correction to camera data
                        tex.Create();

                        // NOW the pointer is valid
                        var nativeTexPtr = tex.GetNativeTexturePtr();
                        Debug.Log($"RenderTexture native ptr: {nativeTexPtr}");

                        // After creating the RenderTexture and doing the GL.Clear trick:
                        info.previewTexture = tex;

                        // debug logs
                        Debug.Log($"Graphics API: {SystemInfo.graphicsDeviceType}");
                        Debug.Log($"Graphics Device: {SystemInfo.graphicsDeviceName}");
                        StartCoroutine(ReadBackOnePixel(tex)); // test if the RGB values are being read correctly

                        // Pass the CORRECT dimensions to the native side
                        ...
                        StartCoroutine(DelayedStart(device.id, tex));
                    }, null);
                }
                else
                {
                    Debug.LogError($"All format attempts failed. Last error: {resizeResult}");
                    throw new ArgumentException("Could not set any valid video format");
                }
            }
        }

        private IEnumerator ReadBackOnePixel(RenderTexture rt)
        {
            yield return new WaitForSeconds(4f); // let a few frames arrive
            ...
            Debug.Log($"Center pixel RGBA: r={c.r} g={c.g} b={c.b} a={c.a}");
            Destroy(tex);
        }

        private IEnumerator DelayedStart(int deviceId, RenderTexture rt)
        {
            // Wait for render loop to be warm
            yield return new WaitForEndOfFrame();
            yield return new WaitForEndOfFrame();

            var nativePtr = rt.GetNativeTexturePtr();
            Debug.Log($"DelayedStart calling native Start with ptr: {nativePtr}");

            // Store params for the render thread callback
            _pendingStartDeviceId = deviceId;
            _pendingStartPtr = nativePtr;
            _pendingStartWidth = rt.width;
            _pendingStartHeight = rt.height;
            _pendingStart = true;

            // Issue a plugin event that will call Start() from the render thread
            yield return new WaitForEndOfFrame();
            // After this yield, _pendingStart should have been consumed by OnRender
        }

        // Add this method to manually try MJPEG format
        private bool TryMJPEGFormat(UVCDevice device, UInt32 width, UInt32 height)
        {
            Debug.Log($"Attempting MJPEG format at {width}x{height}");

            // MJPEG is 0x000007 - often works when YUY2 doesn't
            int result = Resize(device.id, 0x000007, width, height);

            if (result == 0)
            {
                Debug.Log("MJPEG format successfully set!");
                return true;
            }

            Debug.LogWarning($"Failed to set MJPEG format. Result: {result}");
            return false;
        }
        ...
        private void InitPlugin()
        {
            ...
            // USB permission check using iterator instead of array conversion
            try
            {
                AndroidJavaObject activity = AndroidUtils.GetCurrentActivity();
                AndroidJavaObject usbManagerObj = activity.Call<AndroidJavaObject>("getSystemService", "usb");
                AndroidJavaObject deviceList = usbManagerObj.Call<AndroidJavaObject>("getDeviceList");
                int deviceCount = deviceList.Call<int>("size");
                Debug.Log($"UVC: Found {deviceCount} USB devices attached at init time");

                AndroidJavaObject entrySet = deviceList.Call<AndroidJavaObject>("entrySet");
                ...
                while (iterator.Call<bool>("hasNext"))
                {
                    ...
                    AndroidJavaObject device = entry.Call<AndroidJavaObject>("getValue");
                    string name = device.Call<string>("getDeviceName");
                    int vendorId = device.Call<int>("getVendorId");
                    int productId = device.Call<int>("getProductId");
                    bool hasPermission = usbManagerObj.Call<bool>("hasPermission", device);
                    Debug.Log($"UVC: Device '{name}' VID={vendorId} PID={productId} hasPermission={hasPermission}");
                }
            }
            catch (Exception e)
            {
                Debug.LogError($"UVC: USB permission check failed: {e.Message}");
            }
        }
