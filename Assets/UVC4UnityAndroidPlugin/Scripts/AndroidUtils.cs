...
namespace Serenegiant
{
    ...
    public class AndroidUtils : MonoBehaviour
    {
        ...
        public const string PERMISSION_HORIZON_USB = "horizonos.permission.USB_CAMERA"; // horizonos permission
		...
		*/
        ...
        public static IEnumerator GrantCameraPermission(OnPermission callback)
        {
            ...
            //{
            //    // First request standard camera
            //    yield return GrantPermission(PERMISSION_CAMERA, (p, r) => { });

            //    // Then request Horizon USB Camera permission specifically
            //    yield return GrantPermission(PERMISSION_HORIZON_USB, callback);
            //}
            //else
            //{
            ...
            // 1. Request standard Android camera permission first
            yield return GrantPermission(PERMISSION_CAMERA, callback);

            // 2. Request Quest USB camera permission with proper yield
            string horizonUsbPermission = "horizonos.permission.USB_CAMERA";

            bool alreadyGranted = false;
            try
            {
                using (var unityPermission = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
                using (var activity = unityPermission.GetStatic<AndroidJavaObject>("currentActivity"))
                using (var pm = activity.Call<AndroidJavaObject>("getPackageManager"))
                {
                    int result = pm.Call<int>("checkPermission", horizonUsbPermission, activity.Call<string>("getPackageName"));
                    alreadyGranted = (result == 0); // PackageManager.PERMISSION_GRANTED == 0
                }
            }
            catch (Exception e)
            {
                Debug.LogWarning("Could not check USB permission: " + e.Message);
            }

            if (!alreadyGranted)
            {
                // Use the same GrantPermission coroutine pattern so it actually waits
                yield return GrantPermission(horizonUsbPermission, (perm, result) =>
                {
                    Debug.Log($"horizonos USB permission result: {result}");
                });
            }
